'use strict';
app.controller('customerProfileController', ['$scope', 'cityService', 'stateService', 'neighbourhoodService', 'categoryService', 'addressService', 'serviceService', 'customerService', 'proposalService', 'professionalService','customerAddressService', '$location', '$timeout', 'authService', '$window', 'localStorageService', '$sce', '$filter', function ($scope, cityService, stateService, neighbourhoodService, categoryService, addressService, serviceService, customerService, proposalService,professionalService,customerAddressService, $location, $timeout, authService, $window, localStorageService, $sce, $filter) {
    ...
    customerService.getCustomers().then(function (results) {
        ...
        customerService.getCustomers().then(function (results) {
            ...
            $scope.ClientByEmail = clientlist[0];
        })
    ...
    customerAddressService.getCustomerAddresses(data).then(function (results) {
       
        $scope.ClientAddresses = results.data;
